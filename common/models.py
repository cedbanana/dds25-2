...
class Order:
    id: str
    paid: bool
    # In our internal model, items are stored as a list of (item_id, quantity) tuples.
    ...
    def from_proto(cls, proto: order_pb2.Order) -> "Order":
        ...
        items = [(item.item_id, item.stock) for item in proto.items]
        return cls(
            id=proto.id,
            paid=proto.paid,
            items=items,
...
class Stock:
    ...
    def to_proto(self) -> stock_pb2.Item:
        return stock_pb2.Item(item_id=self.item_id, stock=self.stock, price=self.price)

    @classmethod
    def from_proto(cls, proto: stock_pb2.Item) -> "Stock":
        return cls(item_id=proto.item_id, stock=proto.stock, price=proto.price)
